...
Password-based encryption/decryption uses the v1 file format
(``encrypt_file_with_password`` / ``decrypt_file_with_password``)
which embeds KDF parameters in the file for portability.
...
class CryptoEngine(BaseEngine):
    ...
    def _decrypt(self, ctx: ExecutionContext) -> ExecutionResult:
        ...
        if ctx.password is not None and ctx.key is None and ctx.input_bytes is not None:
            ...
            from filanti.crypto.encryption import EncryptedData
            metadata, ciphertext = parse_encrypted_file(ctx.input_bytes)
